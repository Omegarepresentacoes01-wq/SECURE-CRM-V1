...
// Apenas super_admin acessa faturamentos
...
router.post('/', checkSuperAdmin, (req, res) => {
  ...
  // Busca nome da organização automaticamente
  ...
  const result = db.prepare(`
    INSERT INTO faturamentos (organizacao_id, org_nome, tipo, valor, desconto, total,
...
router.put('/:id', checkSuperAdmin, (req, res) => {
  ...
  const campos = ['organizacao_id','org_nome','tipo','valor','desconto','total','vencimento','status','payment_method','pago_em'];
  const sets = [];
  const params = [];

  campos.forEach(c => { if (d[c] !== undefined) { sets.push(`${c} = ?`); params.push(d[c]); } });

  // Recalcula total se valor ou desconto mudaram
  if ((d.valor !== undefined || d.desconto !== undefined) && !sets.includes('total = ?')) {
    ...
    const v = d.valor ?? atual.valor;
    const desc = d.desconto ?? atual.desconto;
    sets.push('total = ?');
    params.push(v - desc);
  }
